...
function SortArrow({ column, sortKey, sortDir }) {
  if (column !== sortKey) return <span className="sort-arrow sort-inactive">⇅</span>
      ...
      <div className="proc-detail-grid">
        <div className="detail-section">
          ...
          <dl>
            ...
            <dt>State</dt><dd>{stateLabel} ({p.stat})</dd>
            <dt>Threads</dt><dd>{p.threads}</dd>
            <dt>Uptime</dt><dd>{fmtUptime(p.uptimeSec)}</dd>
            <dt>OOM Score</dt><dd className={d.oomScore > 500 ? 'val-high' : d.oomScore > 200 ? 'val-med' : ''}>{d.oomScore}</dd>
            <dt>FDs Open</dt><dd>{d.fdCount}</dd>
          </dl>
        </div>
        <div className="detail-section">
          <h4>🧠 Memory</h4>
          <dl>
            <dt>RSS</dt><dd>{fmtBytes(p.rss * 1024)}</dd>
            <dt>PSS</dt><dd>{fmtBytes((d.pssKb || 0) * 1024)}</dd>
            <dt>VM Size</dt><dd>{fmtBytes((d.vmSizeKb || 0) * 1024)}</dd>
            <dt>VM Peak</dt><dd>{fmtBytes((d.vmPeakKb || 0) * 1024)}</dd>
            <dt>Shared Clean</dt><dd>{fmtBytes((d.sharedCleanKb || 0) * 1024)}</dd>
            <dt>Private Clean</dt><dd>{fmtBytes((d.privateCleanKb || 0) * 1024)}</dd>
            <dt>Private Dirty</dt><dd>{fmtBytes((d.privateDirtyKb || 0) * 1024)}</dd>
          </dl>
        </div>
        <div className="detail-section">
          <h4>💾 I/O</h4>
          <dl>
            <dt>Read</dt><dd>{fmtBytes(d.ioReadBytes || 0)}</dd>
            <dt>Written</dt><dd>{fmtBytes(d.ioWriteBytes || 0)}</dd>
            <dt>Read Syscalls</dt><dd>{(d.ioSyscallsR || 0).toLocaleString()}</dd>
            <dt>Write Syscalls</dt><dd>{(d.ioSyscallsW || 0).toLocaleString()}</dd>
          </dl>
        </div>
        <div className="detail-section">
          <h4>📊 Scheduling</h4>
          <dl>
            <dt>Vol. Ctx Switches</dt><dd>{(d.voluntaryCtxSwitches || 0).toLocaleString()}</dd>
            <dt>Invol. Ctx Switches</dt><dd>{(d.nonvoluntaryCtxSwitches || 0).toLocaleString()}</dd>
            <dt>Cgroup</dt><dd className="detail-mono">{d.cgroup || '—'}</dd>
          </dl>
        </div>
      </div>
      {d.envVars && d.envVars.length > 0 && (
        <div className="detail-section detail-env">
          ...
          <div className="env-list">
            {d.envVars.map((v, i) => <code key={i}>{v}</code>)}
          </div>
        </div>
      )}
  ...
  return (
    <div className="section">
      ...
      <div className="proc-summary">
        ...
        <span className="proc-stat">Threads: <strong>{totalThreads}</strong></span>
      </div>
          ...
          <thead>
            <tr>
              {COLUMNS.map(col => (
                <th
                  key={col.key}
                  style={{ width: col.width, textAlign: col.align, cursor: 'pointer', userSelect: 'none' }}
                  onClick={() => handleSort(col.key)}
                  title={`Sort by ${col.label}`}
                >
                  {col.label} <SortArrow column={col.key} sortKey={sortKey} sortDir={sortDir} />
                </th>
              ))}
            </tr>
          </thead>
