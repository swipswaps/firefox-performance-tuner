...
export default function PreferencesPanel({ preferences, categories, onApply }) {
  ...
  const issueCount = Object.keys(flatPrefs).filter(k => preferences[k] !== flatPrefs[k]).length
    ...
    <div>
      ...
      {categoryEntries.map(([catName, prefs]) => {
        const filteredEntries = Object.entries(prefs).filter(([key, val]) => {
          if (filterLower && !key.toLowerCase().includes(filterLower) && !val.description.toLowerCase().includes(filterLower)) return false
          const actual = preferences[key]
          const isOk = actual !== undefined && actual !== null && actual === val.expected
        ...
        return (
          <div key={catName} className="section" style={{ marginBottom: '12px' }}>
            ...
            {filteredEntries.map(([key, val]) => {
              ...
              return (
                <div key={key} className={`pref-row ${isOk ? 'pref-ok' : isNotSet ? 'pref-warn' : 'pref-error'}`}>
                  <div className="pref-main">
                    <span className={`pref-status ${isOk ? 'status-ok' : isNotSet ? 'status-warn' : 'status-error'}`}>
                      {isOk ? '✓' : isNotSet ? '—' : '✗'}
                    </span>
                    ...
                    <div className="pref-values">
                      <span className="pref-actual">{isNotSet ? 'not set' : actual}</span>
                      {!isOk && <span className="pref-expected">expected: {expected}</span>}
                    </div>
                  </div>
                </div>
              )
      ...
      {issueCount > 0 && (
        <div className="pref-action-box">
          <div className="pref-action-text">
            <strong>⚠ {issueCount} preference{issueCount > 1 ? 's' : ''} differ from recommended values</strong>
            <p>Click "Apply" to write optimized values to user.js, then restart Firefox.</p>
          </div>
          {onApply && (
            <button className="btn-apply" onClick={onApply}>Apply All</button>
          )}
        </div>
      )}
    </div>
